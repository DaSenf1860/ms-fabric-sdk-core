...
class FabricClientCore(FabricClient):
    ...
    def __init__(
        ...
        super().__init__(
            tenant_id, client_id, client_secret, username, password, silent=silent
        )
    ...
    def list_workspaces(self, continuationToken=None):
        ...
        for _ in range(10):
            ...
            if response.status_code not in (200, 429):
                ...
                print(response.text)
                raise Exception(
                    f"Error listing workspaces: {response.status_code},  {response.text}"
                )
            break
        ...
        if "continuationToken" in resp_dict:
            ws_list_next = self.list_workspaces(
    ...
    def get_workspace_by_id(self, id):
        ...
        for _ in range(10):
            ...
            if response.status_code not in (200, 429):
                ...
                print(response.text)
                raise Exception(
                    f"Error getting workspace: {response.status_code} {response.text}"
                )
            break
    ...
    def create_workspace(
        self, display_name, capacity_id=None, description=None, exists_ok=True
    ):
        """Create a workspace"""
        body = dict()
        ...
        for _ in range(10):
            response = requests.post(
                url=url, headers=self.auth.get_headers(), json=body
            )
            if response.status_code == 429:
                print("Too many requests, waiting 10 seconds")
                sleep(10)
            ...
            if response.status_code not in (201, 429):
                if (
                    "errorCode" in ws_dict
                    and ws_dict["errorCode"] == "WorkspaceNameAlreadyExists"
                    and exists_ok
                ):
    ...
    def list_capacities(self, continuationToken=None):
        ...
        if "continuationToken" in resp_dict:
            cap_list_next = self.list_capacities(
                continuationToken=resp_dict["continuationToken"]
            )
    ...
    def create_item(
        self, workspace_id, display_name, type, definition=None, description=None
    ):
        """Create an item in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)

        return ws.create_item(
    ...
    def create_shortcut(self, workspace_id, item_id, path, name, target):
        ws = self.get_workspace_by_id(id=workspace_id)
        return ws.get_item(item_id=item_id).create_shortcut(
    ...
    def get_item_job_instance(self, workspace_id, item_id, job_instance_id):
        ...
        ws = self.get_workspace_by_id(id=workspace_id)
        return ws.get_item(item_id=item_id).get_item_job_instance(
            job_instance_id=job_instance_id
        )

    def run_on_demand_item_job(
        self, workspace_id, item_id, job_type, execution_data=None
    ):
        """Run an on demand job for an item"""
        ws = self.get_workspace_by_id(id=workspace_id)
        return ws.get_item(item_id=item_id).run_on_demand_item_job(
    ...
    def cancel_item_job_instance(self, workspace_id, item_id, job_instance_id):
        ...
        ws = self.get_workspace_by_id(id=workspace_id)
        return (
            ws.get_item(item_id=item_id)
            .get_item_job_instance(job_instance_id=job_instance_id)
            .cancel()
        )

    def commit_to_git(
        self, workspace_id, mode, comment=None, items=None, workspace_head=None
    ):
        """Commit changes to git"""
        ws = self.get_workspace_by_id(id=workspace_id)
        return ws.commit_to_git(
            mode=mode, comment=comment, items=items, workspace_head=workspace_head
        )
    ...
    def git_initialize_connection(self, workspace_id, initialization_strategy):
        """Initialize git"""
        ws = self.get_workspace_by_id(id=workspace_id)
        return ws.git_initialize_connection(
    ...
    def create_data_pipeline(self, workspace_id, display_name, description=None):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_data_pipeline(
    ...
    def get_data_pipeline(
        self, workspace_id, data_pipeline_id=None, data_pipeline_name=None
    ):
        """Get a data pipeline from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_data_pipeline(
            data_pipeline_id=data_pipeline_id, data_pipeline_name=data_pipeline_name
        )
    ...
    def update_data_pipeline(
        self, workspace_id, data_pipeline_id, display_name=None, description=None
    ):
        """Update a data pipeline in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_data_pipeline(data_pipeline_id).update(
            display_name=display_name, description=description
        )
    ...
    def update_environment(
        self, workspace_id, environment_id, display_name=None, description=None
    ):
        """Update an environment in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_environment(
            environment_id, display_name=display_name, description=description
        )
    ...
    def upload_staging_library(self, workspace_id, environment_id, file_path):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_environment(environment_id).upload_staging_library(
            file_path=file_path
        )
    ...
    def delete_staging_library(self, workspace_id, environment_id, library_to_delete):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_environment(environment_id).delete_staging_library(
            library_to_delete
        )
    ...
    def get_eventhouse(self, workspace_id, eventhouse_id=None, eventhouse_name=None):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_eventhouse(
            eventhouse_id=eventhouse_id, eventhouse_name=eventhouse_name
        )
    ...
    def update_eventhouse(
        self, workspace_id, eventhouse_id, display_name=None, description=None
    ):
        """Update an eventhouse in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_eventhouse(
            eventhouse_id, display_name=display_name, description=description
        )
    ...
    def get_eventstream(self, workspace_id, eventstream_id=None, eventstream_name=None):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_eventstream(
            eventstream_id=eventstream_id, eventstream_name=eventstream_name
        )
    ...
    def update_eventstream(
        self, workspace_id, eventstream_id, display_name=None, description=None
    ):
        """Update an eventstream in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_eventstream(
            eventstream_id, display_name=display_name, description=description
        )
    ...
    def create_kql_database(
        self, workspace_id, creation_payload, display_name, description=None
    ):
        """Create a kql database in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_kql_database(
    ...
    def get_kql_database(
        self, workspace_id, kql_database_id=None, kql_database_name=None
    ):
        """Get a kql database from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_kql_database(
            kql_database_id=kql_database_id, kql_database_name=kql_database_name
        )
    ...
    def update_kql_database(
        self, workspace_id, kql_database_id, display_name=None, description=None
    ):
        """Update a kql database in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_kql_database(
            kql_database_id, display_name=display_name, description=description
        )
    ...
    def get_kql_queryset(
        self, workspace_id, kql_queryset_id=None, kql_queryset_name=None
    ):
        """Get a kql queryset from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_kql_queryset(
            kql_queryset_id=kql_queryset_id, kql_queryset_name=kql_queryset_name
        )

    def update_kql_queryset(
        self, workspace_id, kql_queryset_id, display_name=None, description=None
    ):
        """Update a kql queryset in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_kql_queryset(
            kql_queryset_id, display_name=display_name, description=description
        )
    ...
    def update_lakehouse(
        self, workspace_id, lakehouse_id, display_name=None, description=None
    ):
        """Update a lakehouse in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_lakehouse(
            lakehouse_id, display_name=display_name, description=description
        )

    def get_lakehouse(self, workspace_id, lakehouse_id=None, lakehouse_name=None):
        """Get a lakehouse from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_lakehouse(
    ...
    def run_on_demand_table_maintenance(
        self,
        workspace_id,
        lakehouse_id,
        execution_data=None,
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.run_on_demand_table_maintenance(
    ...
    def create_ml_experiment(self, workspace_id, display_name, description=None):
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_ml_experiment(
            display_name=display_name, description=description
        )

    def get_ml_experiment(
        self, workspace_id, ml_experiment_id=None, ml_experiment_name=None
    ):
        """Get an ml experiment from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_ml_experiment(
            ml_experiment_id=ml_experiment_id, ml_experiment_name=ml_experiment_name
        )
    ...
    def update_ml_experiment(
        self, workspace_id, ml_experiment_id, display_name=None, description=None
    ):
        """Update an ml experiment in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_ml_experiment(
            ml_experiment_id, display_name=display_name, description=description
        )
    ...
    def update_ml_model(
        self, workspace_id, ml_model_id, display_name=None, description=None
    ):
        """Update an ml model in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_ml_model(
            ml_model_id, display_name=display_name, description=description
        )
    ...
    def create_notebook(
        self, workspace_id, display_name, definition=None, description=None
    ):
        """Create a notebook in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_notebook(
            display_name=display_name, definition=definition, description=description
        )
    ...
    def update_notebook(
        self, workspace_id, notebook_id, display_name=None, description=None
    ):
        """Update a notebook in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_notebook(
            notebook_id, display_name=display_name, description=description
        )
    ...
    def create_semantic_model(
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_semantic_model(
            display_name=display_name, definition=definition, description=description
        )

    def get_semantic_model(
        self, workspace_id, semantic_model_id=None, semantic_model_name=None
    ):
        """Get a semantic model from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_semantic_model(
            semantic_model_id=semantic_model_id, semantic_model_name=semantic_model_name
        )
    ...
    def get_semantic_model_definition(
        self, workspace_id, semantic_model_id, format=None
    ):
    ...
    def update_semantic_model_definition(
        self, workspace_id, semantic_model_id, definition
    ):
    ...
    def create_spark_job_definition(
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_spark_job_definition(
            display_name=display_name, definition=definition, description=description
        )

    def get_spark_job_definition(
        self, workspace_id, spark_job_definition_id=None, spark_job_definition_name=None
    ):
        """Get a spark job definition from a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_spark_job_definition(
    ...
    def update_spark_job_definition(
        self, workspace_id, spark_job_definition_id, display_name=None, description=None
    ):
        """Update a spark job definition in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_spark_job_definition(
            spark_job_definition_id, display_name=display_name, description=description
        )

    def get_spark_job_definition_definition(
        self, workspace_id, spark_job_definition_id, format=None
    ):
        """Get the definition of a spark job definition"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.get_spark_job_definition_definition(
            spark_job_definition_id, format=format
        )

    def update_spark_job_definition_definition(
        self, workspace_id, spark_job_definition_id, definition
    ):
        """Update the definition of a spark job definition"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_spark_job_definition_definition(
            spark_job_definition_id, definition
        )
    ...
    def update_warehouse(
        self, workspace_id, warehouse_id, display_name=None, description=None
    ):
        """Update a warehouse in a workspace"""
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_warehouse(
            warehouse_id, display_name=display_name, description=description
        )
    ...
    def create_workspace_custom_pool(
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.create_workspace_custom_pool(
            name=name,
    ...
    def update_workspace_custom_pool(
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_workspace_custom_pool(
            pool_id,
            name=name,
            node_family=node_family,
            node_size=node_size,
    ...
    def deploy_stage_content(
        ...
        source_stage_id,
        target_stage_id,
        created_workspace_details=None,
        items=None,
        note=None,
        wait_for_completion=True,
    ):
        ...
        pipeline = DeploymentPipeline.get_pipeline(
            deployment_pipeline_id, auth=self.auth
        )
    ...
    def list_deployment_pipelines(self, continuationToken=None):
        ...
        for _ in range(10):
            ...
            if response.status_code not in (200, 429):
                raise Exception(
                    f"Error listing deployment pipelines: {response.status_code}, {response.text}"
                )
            break
        ...
        if "continuationToken" in resp_dict:
            dep_pipes_next = self.list_deployment_pipelines(
                continuationToken=resp_dict["continuationToken"]
            )
    ...
    def get_deployment_pipeline_stages_items(
        self, pipeline_id, stage_id=None, stage_name=None
    ):
    ...
    def update_spark_settings(
        ...
        ws = self.get_workspace_by_id(workspace_id)
        return ws.update_spark_settings(
